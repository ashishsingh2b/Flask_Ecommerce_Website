...
def product_details(product_id):
    items = get_product_by_id(product_id)  # Assuming this still returns a single product

    if not items:
        flash("Product not found.", "danger")
        return redirect(url_for('admin.shop_items'))  # Corrected to point to the right blueprint

    return render_template('product_details.html', items=items)
